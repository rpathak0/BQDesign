...
import { ChevronLeft, CreditCard, ShieldCheck, Ticket, WalletCards } from "lucide-react";
import { Navbar } from "@/components/layout/navbar";
import { Footer } from "@/components/layout/footer";
import { MobileTabbar } from "@/components/layout/mobile-tabbar";
import { SafeImage } from "@/components/shared/safe-image";
import { ArtistCircle } from "@/components/cards/artist-circle";
import { FollowArtistButton } from "@/components/artists/follow-artist-button";
import { ARTISTS, EVENTS } from "@/data/bqData";
import { getArtistProfile } from "@/data/artistProfiles";
...
export default async function ArtistDetailPage({ params }: ArtistDetailPageProps) {
  ...
  if (!artist || !profile) notFound();

  const upcomingEvents = EVENTS.filter((event) => profile.upcomingEventIds.includes(event.id));
  const similarArtists = ARTISTS.filter((item) => profile.similarArtistIds.includes(item.id));
        ...
        <div className="container mx-auto py-6 md:py-8 px-4 sm:px-5 md:px-6 max-w-7xl">
          <nav className="text-sm mb-4 flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 text-muted-foreground">
              ...
              <Link href={`${base}/artists`} className="hover:text-foreground transition-colors">
                Artists
              </Link>
              <span className="text-muted-foreground/70">&#62;</span>
          ...
          <div className="relative aspect-[16/6] overflow-hidden rounded-2xl border border-border/60 mb-6">
          ...
          {similarArtists.length > 0 && (
            <section className="mb-10">
              <h2 className="text-xl font-display font-bold mb-4">Similar</h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-8 place-items-center">
                {similarArtists.map((item) => (
                  <div key={item.id} className="w-full max-w-[140px] flex justify-center">
                    <ArtistCircle artist={item} />
                  </div>
                ))}
              </div>
            </section>
